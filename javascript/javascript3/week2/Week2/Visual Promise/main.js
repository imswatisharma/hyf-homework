const buttonAtOnce = document.querySelector ('#at-once');
const buttonOneByOne = document.querySelector ('#separate');
...
buttonAtOnce.addEventListener ('click', function () {
  Promise.all ([moveRed (), moveGreen (), moveBlue ()])
    .then (() => console.log ('all moved'))
    .catch (console.log);
});

buttonOneByOne.addEventListener ('click', function () {
  translateOneByOne ();
});
